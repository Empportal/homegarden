...
import com.homegarden.org.config.KafkaProperties;
...
class HomegardenAppKafkaResourceIT {
    ...
    void setup() {
        ...
        HomegardenAppKafkaResource kafkaResource = new HomegardenAppKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/homegarden-app-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/homegarden-app-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
